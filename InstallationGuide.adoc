...
== Create the set of domain actions
...
To call actions from the REPL, we have to create a built-in subject so that we
can have something to grant permission to.
...
[source,clojure]
----
(call-action! "https://site.test/actions/create-person" {:example/name "Alice"})
----
