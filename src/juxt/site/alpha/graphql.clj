...
(defn selmer-args
  [{:keys [argument-values user-id object-value type-k]}]
...
(defn prepare-mutation-entity
  [{:keys [field user-id type-k argument-values site-args] :as opts}
   entity transform]
  (let [type (or (get site-args "objectType") (-> field ::g/type-ref ::g/name))
    ...
    (cond-> entity
      ...
      user-id (assoc :_siteSubject user-id)
        ...
        (let [types-by-name (::schema/types-by-name schema)
              ...
              (merge field-resolver-args
                     {:site-args site-args
                      ...
                      :user-id (get (::pass/claims subject) "sub")
                      :db db})]
