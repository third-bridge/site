...
Data in Site is stored as documents.
...
[source,clojure]
----
{:xt/id "https://example.org/identities/arya"
 :username "arya"
 :password "meow" <1>
...
Users, identities and subjects are described in <<ch-users>>.
...
Think of this as an equation where the 'permission' variable must be satisfied,
in the same way as a mathematical equation might ask for a solution for 𝑥. If
...
* Action rules benefit from strong database consistency. Rules are run only
  ...
  cannot achieve consistency since they are across the network and the
  subsequent loss of availability would be too severe.
