...
----
====
<1> The Action arguments will be updated such that the first (0th) argument is updated so that it contains a `:juxt.site.alpha/type` attribute with the value string `Action`.
<2> The arguments will then be validated according to the Malli schema in `:juxt.pass.alpha.malli/args-schema`. By this point it must be a map containing an `:xt/id` value prefixed with `https://site.test/actions/` and a `:juxt.site.alpha/type` with the constant `"Action"`.
<3> then the arguments will be submitted, as documents, to the database.
<4> This is a Datalog rule which says that the Action will only be allowed if there exists a permission allowing the user to call the Action. The terms `permission` and `subject` will be covered in <<ch-subjects>>.

IMPORTANT: Copy and paste the code in <<example-create-action>> into the REPL and evaluate it (with btn:[Enter]).

=== Installing the transaction function

Actions can be called at the REPL with `do-action`.

This requires that a transaction function be registered with the database.
...
IMPORTANT: Install the transaction function with `(install-do-action-fn!)`
