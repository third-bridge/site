...
* Permissions are what restrict actions to specific users and applications via
  permissions.

* An action's rules specify how permissions determine the outcome of
  authorization decisions.

* Permissions are straight-forward to grant and revoke, as they are represented
  as XT documents, so granting and revoking a permission can be done with XTDB's
...
----
include::../../dev/demo.clj[tag=put-repl-user!,indent=0]
----
