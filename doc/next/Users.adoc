...
(((user, definition)))
An Action is performed by a *User*, who has authenticated with the system.
...
(((subject, relationship to identity)))
...
<<user-model>> shows an example of how these documents might relate in the
database.
...
== Create a User to represent the REPL

To create and call actions from the REPL, we'll install a special User that can only be used
from the REPL.

The `(me)` function returns a special built-in identifier for the REPL user.

.Put a User to represent the REPL
