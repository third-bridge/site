...
[NOTE]
--
I think this whole chapter belongs in Preliminaries - doesn't require Resources

Outstanding work:

. Use an access token from the console to access a private resource
.. Put the access token in the `Authorization: Bearer <token>` header
.. Create a private resource
.. Fish out the token using middleware
.. Apply the action associated with the `:get` method of the private resource
