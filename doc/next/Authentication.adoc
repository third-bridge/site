...
. Site needs to know about the OpenID Provider, including which endpoints to call.
...
When an ID Token is received by Site, the database is queried for a matching
identity.
...
attributes required by the domain.

For example, it may be that the `Identity` document references a particular
individual, such that an individual can have multiple identities.

The schema chosen for the `Identity` documents is important, because it will be
coupled to the rules defined on each action. An action's rules might use facts
in the documents referenced by the Identity document to make decisions, for
example, to determine if the individual belongs to a particular group or has
been assigned a particular role.

If a matching identity is found, Site will create a new `Subject` entity with a
`juxt.pass.alpha/identity` attribute referencing the `Identity` entity.

<<identity-model>> shows an example relationship between a `Subject`, `Identity`
and a `Person` document.
